...
local Window = ZyphraxHub:CreateWindow({
	...
	FileSaveName = "ZyphraxHub/MM2.json",
})
...
local settingsRoot = "ZyphraxHub"
local settingsDir = "ZyphraxHub/MM2/Settings"
...
local function startCoinFarm()
	...
	coinFarmConnection = RunService.Heartbeat:Connect(function()
		...
		pcall(function()
			...
			-- Retarget immediately once a coin is taken/invalid.
...
RunService.Heartbeat:Connect(function()
	...
	if flags.autoShoot then
		pcall(function()
			local now = os.clock()
			if now - lastAutoShootAt < 0.12 then return end
			...
			local behindPos = targetRoot.Position - (targetRoot.CFrame.LookVector * 3.2) + Vector3.new(0, 1.2, 0)
			safeTeleport(CFrame.new(behindPos, targetRoot.Position))

			myRoot = getRoot(LocalPlayer.Character)
			if not myRoot then return end

			local lookPos = targetRoot.Position + (velocity * 0.06)
			...
			local burstTargets = {
				targetRoot.Position + (velocity * 0.02),
				targetRoot.Position + (velocity * 0.06),
				targetRoot.Position + (velocity * 0.10)
			}
			...
			for _, shootPos in ipairs(burstTargets) do
